...
let contactQueryProm = fetch(new Request(API_URL + '/tuliptheclown/contact', { credentials: "include" }))
...
if (window.localStorage.getItem('contact.phoneOrEmail')) {
    phoneOrEmail.setAttribute('value', window.localStorage.getItem('contact.phoneOrEmail'))
}
...
function checkSendable(timeRemaining) {
	...
	button.onclick = function() {
	    ...
	    let newMessageProm = fetch(new Request(API_URL + '/tuliptheclown/message', {
		method: "POST",
		credentials: "include",
		headers: {
...
function timeRemainingMsg(timeRemaining) {
    let msg = document.createElement('p')
    msg.textContent = 'Message Sent - you may send another message in ' + timeLeft(timeRemaining)
